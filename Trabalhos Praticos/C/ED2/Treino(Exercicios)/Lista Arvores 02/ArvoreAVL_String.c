...
tipoNoh *pesquisar(tipoNoh *raiz, char valor[]){

	if (raiz != NULL){
		if (strcmp(valor,raiz->item.valor) == 0){
			return raiz;
		}
		else{
			if (strcmp(valor,raiz->item.valor) > 0){
...
int inserir(tipoNoh **raiz, char valor[]){

    int controle;

	if (*raiz == NULL){
		tipoNoh *nohAuxiliar = alocar();
        strcpy(nohAuxiliar->item.valor,valor);
    ...
    if(strcmp(valor,nohAtual->item.valor) < 0){
        if((controle=inserir(&(nohAtual->esquerda),valor))==1){
            if(fatorBalanceamento(nohAtual)>=2){
                if(strcmp(valor,(*raiz)->esquerda->item.valor) < 0)
                    rotacaoDireita(raiz);
                else
    ...
    else
    {
        if(strcmp(valor,nohAtual->item.valor) > 0){
            if ((controle=inserir(&(nohAtual->direita),valor))==1)
            {
                if(fatorBalanceamento(nohAtual)>=2)
                {
                    if(strcmp(valor,(*raiz)->direita->item.valor) > 0)
                        rotacaoEsquerda(raiz);
                    else
...
int remover(tipoNoh **raiz, char valor[])
{
    int controle;
    ...
    if(strcmp(valor,(*raiz)->item.valor) == 0)
    {
        ...
        else 
        {
            ...
            strcpy((*raiz)->item.valor,nohAuxiliar->item.valor);
